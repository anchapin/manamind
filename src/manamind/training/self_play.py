...
from manamind.core.agent import MCTSAgent, Agent
...
class SelfPlayGame:
    ...
    def add_move(self, state: GameState, action: Action, mcts_policy: np.ndarray) -> None:
...
class SelfPlayTrainer:
    ...
    def _generate_self_play_games(self) -> List[Tuple[GameState, np.ndarray, float]]:
        ...
        logger.info(f"Generated {len(all_examples)} training examples from {games_completed} games")
        return all_examples
    ...
    def _maintain_examples_buffer(self) -> None:
        ...
        if len(self.training_examples) > buffer_size:
            ...
            logger.info(f"Trimmed training buffer to {len(self.training_examples)} examples")
    ...
    def _save_checkpoint(self) -> None:
        ...
        checkpoint_path = checkpoint_dir / f"checkpoint_iteration_{self.current_iteration}.pt"
