...
class PolicyValueNetwork(nn.Module):
    ...
    def __init__(
        ...
        if use_attention:
            self.attention = AttentionLayer(hidden_dim, num_attention_heads, dropout_rate)
    ...
    def predict_action_probs(self, game_state, temperature: float = 1.0) -> torch.Tensor:
...
class PolicyValueLoss(nn.Module):
    ...
    def forward(
        ...
        policy_loss = -torch.sum(target_policy * F.log_softmax(policy_logits, dim=-1), dim=-1)
