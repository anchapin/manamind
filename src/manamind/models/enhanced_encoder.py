...
class CardEmbeddingSystem(nn.Module):
    ...
    def forward(self, card: Card) -> torch.Tensor:
        ...
        if hasattr(card, 'current_power') and card.current_power() is not None:
            power_emb = self.power_embedding(torch.tensor([float(card.current_power())]))
        ...
        if hasattr(card, 'current_toughness') and card.current_toughness() is not None:
            toughness_emb = self.toughness_embedding(torch.tensor([float(card.current_toughness())]))
...
class ZoneEncoder(nn.Module):
    ...
    def __init__(self, config: EncoderConfig, zone_type: str):
        ...
        self.card_embedder = CardEmbeddingSystem(config.card_vocab_size, config.embed_dim)
    ...
    def _aggregate_embeddings(self, embeddings: torch.Tensor, zone: Zone, player_id: int) -> torch.Tensor:
...
class GlobalStateEncoder(nn.Module):
    ...
    def forward(self, game_state: GameState) -> torch.Tensor:
        ...
        stack_features = torch.tensor([
            ...
            0.0, 0.0, 0.0, 0.0  # Reserved for stack content analysis
        ])
...
class StateFusionNetwork(nn.Module):
    ...
    def __init__(self, config: EncoderConfig):
        ...
        # Final fusion layers
        fusion_input_dim = config.hidden_dim * (2 * 6 + 2 + 1)  # zones + players + global
    ...
    def forward(self, 
                ...
                combat_encoding: torch.Tensor = None) -> torch.Tensor:
        ...
        zone_names = ['hand', 'battlefield', 'graveyard', 'library', 'exile', 'command_zone']
        for player_id in [0, 1]:
            for zone_name in zone_names:
                if player_id in zone_encodings and zone_name in zone_encodings[player_id]:
...
class EnhancedGameStateEncoder(nn.Module):
    ...
    def forward(self, game_state: GameState) -> torch.Tensor:
        ...
        for player_id, player in enumerate(game_state.players):
            player_zones = {}
            for zone_name in ['hand', 'battlefield', 'graveyard', 'library', 'exile', 'command_zone']:
