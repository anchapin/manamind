...
class GameState:
    ...
    def compute_hash(self) -> int:
        ...
        hash_components = [
            ...
            tuple(len(getattr(p, zone).cards) for p in self.players 
...
class GameStateEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.zone_encoders = nn.ModuleList([
            nn.LSTM(embed_dim, hidden_dim // 2, batch_first=True, bidirectional=True)
    ...
    def forward(self, game_state: GameState) -> torch.Tensor:
        ...
        zone_types = ['hand', 'battlefield', 'graveyard', 'library', 'exile', 'command_zone']
        ...
        global_encoding = self.global_encoder(torch.tensor(global_features, dtype=torch.float32))
