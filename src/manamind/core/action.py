...
class ActionSpace:
    ...
    def get_legal_actions(self, game_state: GameState) -> List[Action]:
        ...
        if (game_state.active_player == current_player_id and 
            current_player.can_play_land() and
            game_state.phase in ["main", "main2"] and
            len(game_state.stack) == 0):
    ...
    def vector_to_action(self, vector: List[float], game_state: GameState) -> Optional[Action]:
