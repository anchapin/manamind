...
from manamind.core.game_state import GameState, Card, Zone, Player
...
class StateHash:
    ...
    def __post_init__(self):
        self._hash = hash((
            self.turn_number, self.phase, self.active_player, self.priority_player,
            self.player_lives, self.zone_sizes, self.stack_size
        ))
...
class IncrementalStateManager:
    ...
    def pop_action(self) -> Optional[GameState]:
        ...
        if delta.reverse_delta:
            ...
            self._current_state = self._apply_reverse_delta(current, delta.reverse_delta)
            return self._current_state
    ...
    def _compute_delta(self, action: Action, old_state: GameState, new_state: GameState) -> StateDelta:
        """Compute delta between states."""
        delta = StateDelta(action=action, timestamp=action.timestamp)
        
        # Compare players
        for i, (old_player, new_player) in enumerate(zip(old_state.players, new_state.players)):
            changes = {}
        ...
        if old_state.priority_player != new_state.priority_player:
            delta.global_changes['priority'] = (old_state.priority_player, new_state.priority_player)
        
        return delta
    
    def _apply_reverse_delta(self, state: GameState, reverse_delta: StateDelta) -> GameState:
...
class BatchStateProcessor:
    ...
    def process_states_parallel(self, 
    ...
    def _batch_encode(self, states: List[GameState], encoder=None) -> List[torch.Tensor]:
    ...
    def _batch_legal_actions(self, states: List[GameState], action_space=None) -> List[List[Action]]:
...
class TranspositionTable:
    ...
    def update_mcts_data(self, state_hash: StateHash, visit_count: int, 
                        value_estimate: float, action_values: Dict[str, float]):
