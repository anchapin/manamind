...
class TestCard:
    ...
    def test_card_equality(self):
        ...
        card1 = Card(name="Lightning Bolt", card_id=1)
        card2 = Card(name="Lightning Bolt", card_id=1)
        card3 = Card(name="Grizzly Bears", card_id=2)
    ...
    def test_card_hash(self):
        """Test card hashing."""
        card = Card(name="Lightning Bolt", card_id=1)
...
class TestZone:
    ...
    def test_zone_iteration(self):
        """Test zone iteration."""
        zone = Zone(name="graveyard", owner=0)
        cards = [Card(name=f"Card {i}") for i in range(3)]
...
class TestPlayer:
    ...
    def test_player_draw_cards(self):
        ...
        cards = [Card(name=f"Card {i}") for i in range(5)]
        for card in cards:
        ...
        assert player.library.size() == 2

    def test_player_lose_condition(self):
        """Test player lose condition."""
        player = Player(player_id=0)
...
class TestGameState:
    ...
    def test_standard_game_start(self):
        """Test standard game start creation."""
        game_state = create_standard_game_start()
        
        # Should have 7 cards in hand for each player
        assert game_state.players[0].hand.size() == 7
        assert game_state.players[1].hand.size() == 7
        
        # Should have cards in library
        assert game_state.players[0].library.size() > 0
        assert game_state.players[1].library.size() > 0

    def test_game_state_copy(self):
        """Test game state copying."""
        game_state = create_empty_game_state()
        copied = game_state.copy()
        
        assert copied.turn_number == game_state.turn_number
        assert copied.phase == game_state.phase
        assert len(copied.players) == len(game_state.players)
...
class TestGameStateEncoder:
    ...
    def encoder(self):
        ...
        return GameStateEncoder(
            vocab_size=1000, embed_dim=64, hidden_dim=128, output_dim=256
        )
